<script lang="ts">
  ...
  $: etape =
    situtation.Etapes[$page.params.etape as keyof typeof situtation.Etapes];
...
<div class="container">
  ...
  <div class="verif">
    ...
    {#if right === true}
      <div class="next">
        <button class="btn success" on:click={() => nextStage()}
          >Prochaine étape</button
        >
      </div>
    {:else if right === false}
      <div class="false">
        <button class="btn error" on:click={() => resetAnswers()}
          >Réssayer</button
        >
      </div>
    {/if}
